...
def HodgkinHuxley(I,preVoltage):
    ...
    '''def synAlpha(preV,postV):
        return 0.4*0.5*(1+np.tanh(5*(preV-postV)))'''
    ...
    for i in range(1,len(t)):
        ...
        s.append(s[i-1] + dt*((alpha_s(preVoltage[i-1])*(1-s[i-1]) - beta_s(preVoltage[i-1])*s[i-1])))
